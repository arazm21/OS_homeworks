...
kinit()
{
  ...
  superrange((void*)SUPERSTART,(void*)PHYSTOP);
}
...
superfree(void *pa){
  ...
  s=(struct superpage* )pa;
  acquire(&kmem.lock);
  s->next = kmem.superlist;
  kmem.superlist = s;
  release(&kmem.lock);
...
superalloc(void){
  ...
  s=kmem.superlist;
  if(s)
    kmem.superlist = s->next;
  release(&kmem.lock);
  if(s)
    memset((char*)s,5,SUPERPGSIZE);
  return (void*)s;
